    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
        {payrollEntries.map((entry) => (
          <Card key={entry.id}>
            <CardHeader>
              <CardTitle>{entry.employees.first_name} {entry.employees.last_name}</CardTitle>
            </CardHeader>
            <CardContent>
              <p>Pay Period: {new Date(entry.pay_period_start).toLocaleDateString()} - {new Date(entry.pay_period_end).toLocaleDateString()}</p>
              <p>Base Salary: ${entry.base_salary.toFixed(2)}</p>
              <p>Deductions: ${entry.deductions.toFixed(2)}</p>
              <p>Net Pay: ${entry.net_pay.toFixed(2)}</p>
              <p>Payment Date: {new Date(entry.payment_date).toLocaleDateString()}</p>
            </CardContent>
          </Card>
        ))}
      </div>
